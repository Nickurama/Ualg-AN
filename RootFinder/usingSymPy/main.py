...
while exit == false:
    ...
    try:  # ONLY WORKS IF THERE IS ONE AND ONLY ONE ROOT INSIDE RANGE
        ...
        range1 = math_input.math_input.get_range1(
            range0, precision
        )  # range1 > range0 && função tem de ser contínua [range0, range1]
        error_margin = math_input.math_input.get_error_margin(
            precision
        )  # can be 0 (but should set max_iterations)
        max_iterations = math_input.math_input.get_max_iterations(
            precision
        )  # can be oo (but should set error_margin)
        try:
            print(">-- root (bisector) --<")
            result_bisector = bisector.bisector.calc_root_nonrecursive(
                expression, range0, range1, error_margin, max_iterations
            )
            print(
                f"result: {round(result_bisector, precision)}"
        ...
        try:
            print(">-- root (Newton) --<")
            result_newton = newton.newton.calc_root_nonrecursive(
                expression, (range0 + range1) / 2, error_margin, max_iterations
            )  # point is (range0 + range1) / 2
